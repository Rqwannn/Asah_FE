			...
			<div className="flex-1">
				<div className="h-full px-8 py-6 mr-[310px] overflow-y-auto">
					...
					<div className="flex flex-col gap-1">
						<h1 className="text-[32px] leading-8 font-bold text-[#202020]">Daily Check-in</h1>
						...
						<CardHeader className="pb-2 pt-6 px-8">
							<CardTitle className="flex justify-between items-center text-[24px] font-semibold text-[#202020]">
								...
								<div className="flex gap-1">
									<Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-gray-50 text-gray-400" disabled><i className="ri-arrow-left-s-line text-xl"></i></Button>
									<Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-gray-50 text-gray-400" disabled><i className="ri-arrow-right-s-line text-xl"></i></Button>
								</div>
							</CardTitle>
						</CardHeader>
										...
										<div key={i} className="aspect-square relative group">
											<button
												...
												className={`
                                                    ...
                                                    ${isToday && !checkin ? 'border-2 border-[#285F3E] text-[#285F3E] font-bold' : ''}
                                                    ${checkin
											...
											>
												...
												{checkin && (
													<div className="flex items-center justify-center mt-0.5">
														<i className={`${moods.find(m => m.value === checkin.mood)?.icon} text-lg`}></i>
													</div>
												)}
											</button>
										</div>
								...
								{viewMode === 'details' && selectedCheckin ? (
									<div className="flex flex-col gap-5">
										...
										<div className="flex flex-col items-center gap-3 py-2">
											<div className={`p-5 rounded-full ${moods.find(m => m.value === selectedCheckin.mood)?.bg || 'bg-gray-100'} flex items-center justify-center aspect-square`}>
												<i className={`${moods.find(m => m.value === selectedCheckin.mood)?.icon || 'ri-emotion-normal-line'} text-[40px] ${moods.find(m => m.value === selectedCheckin.mood)?.color || 'text-gray-500'}`}></i>
											</div>
										...
										<div className="flex flex-col gap-2">
											<span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Your Thoughts</span>
										...
										<div className="grid grid-cols-5 gap-3">
											{moods.map((m) => (
												<button
													...
													title={m.label}
												>
													<i className={`${m.icon} text-[32px] ${m.color}`}></i>
												</button>
											))}
										</div>
