...
export class JourneyRepositoryImpl implements JourneyRepository {
	...
	async getJourneys(): Promise<JourneyDTO[]> {
		const response = await this.http.get<
			ApiResponseDTO<{ journeys: JourneyDTO[] }>
		>("/journeys");
	...
	async getJourney(id: string): Promise<JourneyDTO> {
		const response = await this.http.get<
			ApiResponseDTO<{ journey: JourneyDTO }>
		>(`/journeys/${id}`);
		return response.data.data.journey;
	}

	async getJourneyTrackings(id: string): Promise<JourneyTrackingResponseDTO[]> {
		const response = await this.http.get<
			ApiResponseDTO<JourneyTrackingResponseDTO[]>
		>(`/journeys/${id}/tutorials/trackings`);
